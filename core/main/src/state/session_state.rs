...
mod tests {
    ...
    #[tokio::test]
    async fn test_send_json_rpc_success() {
        let (tx, mut rx) = mpsc::channel(1);
        let session = Session::new("test_app".to_string(), Some(tx));
        let msg = ApiMessage::new(
            ApiProtocol::JsonRpc,
            "{\"jsonrpc\":\"2.0\",\"method\":\"test\",\"id\":1}".to_string(),
            "test_id".to_string(),
        );

        let result = session.send_json_rpc(msg.clone()).await;
        assert!(result.is_ok());
        // Ensure the message was actually sent
    ...
    #[tokio::test]
    async fn test_send_json_rpc_no_sender() {
        let session = Session::new("test_app".to_string(), None);
        let msg = ApiMessage::new(
            ApiProtocol::JsonRpc,
            "{\"jsonrpc\":\"2.0\",\"method\":\"test\",\"id\":1}".to_string(),
            "test_id".to_string(),
        ...
        let result = session.send_json_rpc(msg).await;
        assert_eq!(result, Err(RippleError::SendFailure));
    }

    #[tokio::test]
    async fn test_send_json_rpc_send_failure() {
        // Drop receiver immediately to cause send failure
    ...
    #[test]
    fn test_get_app_id() {
        let session = Session::new("my_app_id".to_string(), None);
        assert_eq!(session.get_app_id(), "my_app_id".to_string());
    }
}
