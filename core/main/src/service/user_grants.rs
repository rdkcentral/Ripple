...
impl UserGrantStateUtils {
    ...
    pub async fn grant_modify(
        platform_state: &PlatformState,
        modify_opertation: GrantStateModify,
        app_id: Option<String>,
        role: CapabilityRole,
        capability: String,
    ) -> bool {
        ...
        // using block here to restrict the grant_state's lock scope.
        {
            let mut grant_state = platform_state.grant_state.grant_map.write().unwrap();
            ...
            if let Some(grant_policy) =
                UserGrants::retrieve_grant_policy_from_manifest(platform_state, &permission)
            {
                // Do the scope validation here and return false, if there is any scope mismatch.
                if app_id.is_some() && grant_policy.scope != Scope::App {
                    return false;
                }
                ...
                // Removing the existing entry from the GrantEntry Set.
                if entries.remove(&new_entry) {
                    debug!("GrantEntry {:?} Removed from GrantEntry Set", new_entry);
                }

                match modify_opertation {
                ...
                error!(
                    "No matching grant entry found in manifest for the given permission {:?} ",
                    permission
                );
            }
        }

        // calling persist_user_grants async function after unlocking the grant_state.
        if entry_modified {
            let _ = UserGrantStateUtils::persist_user_grants(platform_state).await;
        };
        entry_modified
...
impl UserGrants {
    ...
    pub fn retrieve_grant_policy_from_manifest(
        platform_state: &PlatformState,
        ...
        let grant_policy = match permission.role {
    ...
    pub async fn check_if_all_caps_available(
        ...
        req_caps: &Vec<FireboltPermission>,
    ) -> bool {
        let caps: Vec<String> = req_caps.into_iter().map(|p| p.cap.as_str()).collect();
        UserGrants::are_caps_available(platform_state, &caps).await
    }
    ...
    pub async fn get_grant_policy(
        ...
        let mut res = Self::retrieve_grant_policy_from_manifest(platform_state, permission);
        if let Some(grant_policy) = res.as_mut() {
    ...
    pub async fn determine_grant_policies_for_permission(
        ...
        if grant_policy_opt.is_none() {
            tracing::debug!("There are no grant policies for the requesting cap so bailing out");
            return Ok(());
        }
        let grant_policy = grant_policy_opt.unwrap();
        if !grant_policy.is_valid(platform_state).await {
            return Err(DenyReason::Disabled);
        }
        let result = grant_policy
...
pub(crate) mod tests {
    ...
    pub fn start_local_storage_service(mut rx: tokio::sync::mpsc::Receiver<DabRequest>) {
        tokio::spawn(async move {
