...
pub async fn initialize(&self, mut tr: mpsc::Receiver<ApiMessage>) {
    ...
    let base_path = std::env::var("RIPPLE_SERVICE_HANDSHAKE_PATH")
        .unwrap_or_else(|_| "127.0.0.1:3474".to_string());
    let path = match tokio_tungstenite::tungstenite::http::Uri::builder()
        .scheme("ws")
        .authority(base_path.as_str())
        .path_and_query(format!("/?service_handshake={}", self.sender.get_cap()))
        .build()
    {
        ...
        Err(e) => {
            error!("Failed to build WebSocket URI: {:?}", e);
            return;
        }
    };

    match WebSocketUtils::get_ws_stream(&path, None).await {
        Ok((mut ws_tx, mut ws_rx)) => {
            tokio::pin! {
                let mut read_pin = ws_rx.next();
            }

            loop {
                tokio::select! {
                    Some(value) = &mut read_pin => {
                        if let Err(e) = self.handle_incoming_message(value).await {
                            error!("Error handling incoming message: {:?}", e);
                            break;
                        }
                        read_pin = ws_rx.next(); // Re-pin the future
                    },
                    Some(request) = tr.recv() => {
                        if let Err(e) = self.handle_outgoing_message(&mut ws_tx, request).await {
...
async fn handle_incoming_message(
    &self,
    value: Result<Message, tokio_tungstenite::tungstenite::Error>,
) -> Result<(), RippleError> {
    match value {
        Ok(msg) => {
            if let Message::Text(message) = msg {
                match ExtnMessage::try_from(message) {
                    Ok(extn_message) => {
                        if let Some(ts) = extn_message.ts {
                            let latency = Utc::now().timestamp_millis() - ts;
                            if latency > self.get_latency_threshold() {
                                warn!("High IEC Latency: {:?}", latency);
                            }
                        }
                        self.handle_message(extn_message);
                    }
        ...
        Err(e) => {
            error!("WebSocket error on read: {:?}", e);
            return Err(RippleError::WebSocketError);
        }
    }
    Ok(())
}
...
async fn handle_outgoing_message(
    &self,
    ws_tx: &mut impl SinkExt<Message>,
    request: ApiMessage,
) -> Result<(), RippleError> {
    trace!("IEC send: {:?}", request.jsonrpc_msg);
    ws_tx.feed(Message::Text(request.jsonrpc_msg)).await?;
    ws_tx.flush().await?;
    Ok(())
}

fn get_latency_threshold(&self) -> i64 {
...
impl ExtnClient {
    pub fn handle_message(&self, message: ExtnMessage) -> ControlFlow<()> {
        ...
        if message.payload.is_response() {
            ...
            return self.handle_event_message(message);
        } else {
            return self.handle_request_message(message);
        }
    }

    fn handle_response_message(&self, message: ExtnMessage) -> ControlFlow<()> {
        Self::handle_single(message, self.response_processors.clone());
        ControlFlow::Continue(())
    }

    fn handle_event_message(&self, message: ExtnMessage) -> ControlFlow<()> {
        if self.sender.get_cap().is_main() && self.should_forward_event(&message) {
            self.forward_message_to_target(message);
        } else {
            Self::handle_vec_stream(message, self.event_processors.clone());
        }
        ControlFlow::Continue(())
    ...
    fn handle_request_message(&self, message: ExtnMessage) -> ControlFlow<()> {
        if self.sender.get_cap().is_main() {
            if !Self::handle_stream(message.clone(), self.request_processors.clone()) {
                self.handle_no_processor_error(message);
            }
    ...
    fn should_forward_event(&self, message: &ExtnMessage) -> bool {
        message.target_id.is_some() && !message.target_id.as_ref().unwrap().is_main()
    }

    fn forward_message_to_target(&self, message: ExtnMessage) {
        if let Some(sender) = self.get_extn_sender_with_extn_id(&message.target_id.as_ref().unwrap().to_string()) {
            let send_response = self.sender.respond(message, Some(sender));
            trace!("Forwarding event result: {:?}", send_response);
        } else {
            error!("Unable to get sender for target: {:?}", message.target_id);
            self.handle_no_processor_error(message);
        }
