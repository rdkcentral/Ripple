...
use ripple_sdk::{
    api::gateway::rpc_gateway_api::JsonRpcApiResponse, chrono::{DateTime, Duration, Utc}, framework::RippleResponse, log::{error, info, warn}, tokio::sync::mpsc::Sender, utils::error::RippleError
};
...
impl StatusManager {
    ...
    pub fn add_broker_request_to_pending_list(&self, plugin_name: String, request: ThunderAsyncRequest) {
        let mut status = self.status.write().unwrap();
        if let Some(plugin_state) = status.get_mut(&plugin_name) {
